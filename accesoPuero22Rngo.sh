...
for (( ip_num=$start_num; ip_num<=$end_num; ip_num++ )); do
  ip=$(num_to_ip $ip_num)
  sudo iptables -D INPUT -p tcp --dport 22 -s $ip -j DROP
done
echo "Eliminación de la regla de bloqueo del puerto 22 para el rango de IPs $START_IP a $END_IP completada.""